...
void USuturingController::InitController()
{
	...
	if (bHapticTool) {
		...
		RigidController->setTranslationOffset(UMathUtil::ToImstkVec3d(GetComponentLocation(), true));

		std::shared_ptr<imstk::DeviceManager> HapticManager = imstk::DeviceManagerFactory::makeDeviceManager();
		HapticDeviceClient = HapticManager->makeDeviceClient();
		RigidController->setDevice(HapticDeviceClient);
