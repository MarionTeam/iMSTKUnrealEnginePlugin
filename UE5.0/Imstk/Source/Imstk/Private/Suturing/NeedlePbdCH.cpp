...
void
NeedlePbdCH::init(std::shared_ptr<imstk::PbdObject> threadObj)
{
	// Setup pbd tissue object
	m_pbdTissueObj = std::dynamic_pointer_cast<imstk::PbdObject>(getInputObjectA());
	auto physMesh = std::dynamic_pointer_cast<imstk::TetrahedralMesh>(m_pbdTissueObj->getPhysicsGeometry());
	m_meshVerticesPtr = physMesh->getVertexPositions();

	// Get surface mesh
	m_tissueSurfMesh = std::dynamic_pointer_cast<imstk::SurfaceMesh>(m_pbdTissueObj->getCollidingGeometry());

	// set up thread mesh
	m_threadObj = threadObj;
	m_threadMesh = std::dynamic_pointer_cast<imstk::LineMesh>(m_threadObj->getCollidingGeometry());
	...
	const std::vector<imstk::CollisionElement>& elementsB)         //override
{
	...
	auto needleRigid = std::dynamic_pointer_cast<imstk::RigidBody>(getInputObjectB());

	// Setup needle and get collision state
	auto needleObj = std::dynamic_pointer_cast<NeedleObject>(getInputObjectB());
	auto needleMesh = std::dynamic_pointer_cast<imstk::LineMesh>(needleObj->getCollidingGeometry());

	std::shared_ptr<imstk::VecDataArray<double, 3>> needleVerticesPtr = needleMesh->getVertexPositions();
	imstk::VecDataArray<double, 3>& needleVertices = *needleVerticesPtr;

	// Unpack thread data

	// Get thread vertex positions
	imstk::VecDataArray<double, 3>& threadVertices = *m_threadVerticesPtr;

	// Get thread velocities
	std::shared_ptr<imstk::PointSet> pointSetA = std::dynamic_pointer_cast<imstk::PointSet>(m_threadObj->getPhysicsGeometry());
	auto                      threadVelocitiesPtr = std::dynamic_pointer_cast<imstk::VecDataArray<double, 3>>(pointSetA->getVertexAttribute("Velocities"));
	imstk::VecDataArray<double, 3>& threadVelocity = *threadVelocitiesPtr;

	// Unpack tissue data

	// Get vertex positions
	imstk::VecDataArray<double, 3>& meshVertices = *m_meshVerticesPtr;

	// For something to be a PbdObject it must have a pointset, it must also have invMasses defined
	std::shared_ptr<imstk::PointSet>                pointSetB = std::dynamic_pointer_cast<imstk::PointSet>(m_pbdTissueObj->getPhysicsGeometry());
	const std::shared_ptr<imstk::DataArray<double>> triangleInvMassesAPtr = std::dynamic_pointer_cast<imstk::DataArray<double>>(pointSetB->getVertexAttribute("InvMass"));
	const imstk::DataArray<double>& triangleInvMasses = *triangleInvMassesAPtr;

	// Get velocities
	std::shared_ptr<imstk::VecDataArray<double, 3>> velocitiesAPtr = std::dynamic_pointer_cast<imstk::VecDataArray<double, 3>>(pointSetB->getVertexAttribute("Velocities"));
	imstk::VecDataArray<double, 3>& meshVelocity = *velocitiesAPtr;
	...
	auto one2one = std::dynamic_pointer_cast<imstk::PointwiseMap>(m_pbdTissueObj->getPhysicsToCollidingMap());
	...
	std::shared_ptr<imstk::VecDataArray<int, 2>> needleIndicesPtr = needleMesh->getCells();
	const imstk::VecDataArray<int, 2>& needleIndices = *needleIndicesPtr;
	std::shared_ptr<imstk::VecDataArray<int, 3>> tissueSurfMeshIndicesPtr = m_tissueSurfMesh->getCells();
	const imstk::VecDataArray<int, 3>& tissueSurfMeshIndices = *tissueSurfMeshIndicesPtr;
	std::shared_ptr<imstk::VecDataArray<int, 2>> threadIndcicesPtr = m_threadMesh->getCells();
	const imstk::VecDataArray<int, 2>& threadIndices = *threadIndcicesPtr;
	...
	if (needleObj->getCollisionState() == NeedleObject::CollisionState::INSERTED)
	{
		// Scope for needle
		{
			...
			imstk::Vec2i       nodeIds = needleIndices[tipSegmentId];
			const imstk::Vec3d tip1 = needleVertices[nodeIds[0]];
			const imstk::Vec3d tip2 = needleVertices[nodeIds[1]];

			// Loop over all triangles in the surface mesh
			for (int triangleId = 0; triangleId < m_tissueSurfMesh->getNumCells(); triangleId++)
			{
				const imstk::Vec3i& surfTriIds = tissueSurfMeshIndices[triangleId];

				// Indices of the vertices on the physics mesh
				imstk::Vec3i physTriIds;
				...
				const imstk::Vec3d& a = meshVertices[physTriIds[0]];
				const imstk::Vec3d& b = meshVertices[physTriIds[1]];
				const imstk::Vec3d& c = meshVertices[physTriIds[2]];

				// Barycentric coordinates of interseciton point
				imstk::Vec3d uvw = imstk::Vec3d::Zero();

				// If this triangle has not already been punctured
				if (m_isNeedlePunctured[triangleId] == false)
				{
					// Check for intersection
					if (imstk::CollisionUtils::testSegmentTriangle(tip1, tip2, a, b, c, uvw))
					{
			...
			for (int pPointId = 0; pPointId < m_needlePData.size(); pPointId++)
			{
				...
				imstk::Vec3d            closestPoint = { maxVal, maxVal, maxVal };
				double           closestDist = IMSTK_DOUBLE_MAX;

				const imstk::Vec3d& a = meshVertices[m_needlePData[pPointId].triVertIds[0]];
				const imstk::Vec3d& b = meshVertices[m_needlePData[pPointId].triVertIds[1]];
				const imstk::Vec3d& c = meshVertices[m_needlePData[pPointId].triVertIds[2]];

				const imstk::Vec3d& baryPoint = m_needlePData[pPointId].triBaryPuncturePoint;
				const imstk::Vec3d  puncturePt = baryPoint[0] * a + baryPoint[1] * b + baryPoint[2] * c;

				for (int segmentId = 0; segmentId < needleMesh->getNumCells(); segmentId++)
				{
					const imstk::Vec2i& needleSegNodeIds = needleIndices[segmentId];
					const imstk::Vec3d& x1 = needleVertices[needleSegNodeIds[0]];
					const imstk::Vec3d& x2 = needleVertices[needleSegNodeIds[1]];

					int caseType = -1;

					// Find the closest point on this segment
					const imstk::Vec3d segClosestPoint = imstk::CollisionUtils::closestPointOnSegment(puncturePt, x1, x2, caseType);
				...
				imstk::Vec3d diffTail = closestPoint - needleVertices[0];
				imstk::Vec3d diffTip = closestPoint - needleVertices[needleMesh->getNumVertices() - 1];
		...
		{     // Scope for thread
			  ...
			  imstk::Vec3d threadTip = threadVertices[0];

			// First, find new penetration points
			const imstk::Vec2i& nodeIds = threadIndices[0];
			const imstk::Vec3d& threadTip1 = threadVertices[nodeIds[0]];
			const imstk::Vec3d& threadTip2 = threadVertices[nodeIds[1]];
			...
			for (int triangleId = 0; triangleId < m_tissueSurfMesh->getNumCells(); triangleId++)
			{
				...
				if (m_isNeedlePunctured[triangleId] == true)
				{
					...
					{
						// Check for intersection
						if (imstk::CollisionUtils::testSegmentTriangle(threadTip1, threadTip2, a, b, c, uvw) == true)
						{
			...
			for (int pPointId = 0; pPointId < m_threadPData.size(); pPointId++)
			{
				...
				imstk::Vec3d closestPoint = { IMSTK_DOUBLE_MAX, IMSTK_DOUBLE_MAX, IMSTK_DOUBLE_MAX };

				const imstk::Vec3d& a = meshVertices[m_threadPData[pPointId].triVertIds[0]];
				const imstk::Vec3d& b = meshVertices[m_threadPData[pPointId].triVertIds[1]];
				const imstk::Vec3d& c = meshVertices[m_threadPData[pPointId].triVertIds[2]];

				imstk::Vec3d baryPoint = m_threadPData[pPointId].triBaryPuncturePoint;
				...
				{
					const imstk::Vec2i& threadSegNodeIds = threadIndices[segmentId];
					const imstk::Vec3d& x1 = threadVertices[threadSegNodeIds[0]];
					const imstk::Vec3d& x2 = threadVertices[threadSegNodeIds[1]];

					int caseType = -1;

					const imstk::Vec3d segClosestPoint = imstk::CollisionUtils::closestPointOnSegment(puncturePt, x1, x2, caseType);

					const imstk::Vec3d newDist = segClosestPoint - puncturePt;
					const imstk::Vec3d oldDist = closestPoint - puncturePt;
				...
				const imstk::Vec3d diffTip = closestPoint - threadVertices[0];
				const imstk::Vec3d diffTail = closestPoint - threadVertices[m_threadMesh->getNumVertices() - 1];
				...
				const imstk::Vec2i& nearestSegNodeIds = threadIndices[closestSegmentId];
				const imstk::Vec3d& p = threadVertices[nearestSegNodeIds[0]];
				const imstk::Vec3d& q = threadVertices[nearestSegNodeIds[1]];
				...
				const imstk::Vec2d segBary = imstk::baryCentric(closestPoint, p, q);
...
NeedlePbdCH::stitch()
{
	...
	if (m_threadPData.size() >= 4)
	{
		...
		for (int pPointId = 0; pPointId < m_threadPData.size(); pPointId++)
		{
			...
			std::vector<imstk::VertexMassPair> ptsA = { ptA1, ptA2, ptA3 };
			...
			std::vector<imstk::VertexMassPair> ptsB = { centerPt };
			std::vector<double>         weightsB = { 1.0 };

			auto constraint = std::make_shared<imstk::PbdBaryPointToPointConstraint>();
	...
	double stiffnessA, double stiffnessB)
{
	...
	imstk::Vec3d surfNormal = imstk::Vec3d::Zero();

	// Note: assumes closed mesh

	// Assuming traingle has points a,b,c
	imstk::Vec3d ab = *ptB2.vertex - *ptB1.vertex;
	imstk::Vec3d ac = *ptB3.vertex - *ptB1.vertex;
