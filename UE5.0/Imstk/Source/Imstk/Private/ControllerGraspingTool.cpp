...
EControllerObjectType UControllerGraspingTool::Init(UImstkController* Controller, const FControllerToolFilter& ToolFilter)
{
	...
	if (Controller->ToolGeomFilter.GeomType != EGeometryType::Sphere && Controller->ToolGeomFilter.GeomType != EGeometryType::Capsule) {
		SubsystemInstance->LogToUnrealAndImstk("Error: Controller type to Geometry mismatch for " + Controller->GetName());
	...
	//GraspStiffness = ToolFilter.GraspingToolStruct.GraspStiffness;
...
bool UControllerGraspingTool::CreateInteraction(UDynamicalModel* OtherObject)
{
	if (std::shared_ptr<imstk::PbdObject> DeformableObj = std::dynamic_pointer_cast<imstk::PbdObject>(OtherObject->ImstkCollidingObject)) {
		...
		else if (ControllerToolFilter.GraspingToolStruct.GraspType == EGraspType::RayPointGrasp || ControllerToolFilter.GraspingToolStruct.GraspType == EGraspType::CellGrasp) {
