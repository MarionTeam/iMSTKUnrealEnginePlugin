...
std::shared_ptr<imstk::SceneObject> UControllerBurningTool::CreateInteraction(UDynamicalModel* OtherObject)
{
	...
	if (Connective) {
		...
		std::dynamic_pointer_cast<imstk::PbdObject>(Connective->ImstkCollidingObject)->addComponent(Burnable);
		...
		Burner->setOnTime(ControllerToolFilter.BurningToolStruct.OnTime);

		//Burner->setBurnerGeometry(std::dynamic_pointer_cast<imstk::AnalyticalGeometry>(ControllerComponent->GetToolObj()->getCollidingGeometry()));
