...
void UImstkSubsystem::ImstkInit()
{
	...
	if (UImstkSettings::IsWritingTaskGraph()) {
		...
		FSavePackageArgs SavePackageArgs = {NULL, NULL,  EObjectFlags::RF_Standalone ,0U, false, true, true, FDateTime::MinValue(), GError };
...
void UImstkSubsystem::UpdateSimulation(float DeltaTime)
{
	if (bIsInitialized) {
		...
		if (UImstkSettings::IsUseRealtimeRBD()) 
			//RigidBodyModel->getConfig()->m_dt = DeltaTime;
			RigidBodyModel->getConfig()->m_dt = SceneManager->getDt();
		if (UImstkSettings::IsUseRealtimePBD())
			//PbdModel->getConfig()->m_dt = DeltaTime;
			PbdModel->getConfig()->m_dt = SceneManager->getDt();

		if (UImstkSettings::IsUseSubstepping()) {
