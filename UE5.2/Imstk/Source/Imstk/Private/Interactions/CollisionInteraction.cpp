...
void UCollisionInteraction::Init()
{
	...
	if (CollisionType == "") {
		if (Cast<UPBDThread>(Model1) && Cast<UPBDThread>(Model2)) {
			...
			std::dynamic_pointer_cast<imstk::LineMeshToLineMeshCCD>(Interaction->getCollisionDetection())->setThickness(Thickness);
		}
...
void UCollisionInteraction::DisableInteraction() {
	for (auto Interaction : Interactions)
		std::dynamic_pointer_cast<imstk::CollisionInteraction>(Interaction)->setEnabled(false);
}


void UCollisionInteraction::EnableInteraction() {
	for (auto Interaction : Interactions)
		std::dynamic_pointer_cast<imstk::CollisionInteraction>(Interaction)->setEnabled(true);
}