...
void UConnectiveTissue::Init()
{
	...
	if (!PbdModelA || !PbdModelB) {
		SubsystemInstance->LogToUnrealAndImstk("Models not assigned on connective tissue.", FColor::Red);
		return;
	}

	if (!PbdModelA->bSharedModel || !PbdModelB->bSharedModel) {
		SubsystemInstance->LogToUnrealAndImstk("One or both of the models on connective tissue are not using a shared model.", FColor::Red);
		return;
	}
	...
	if (bIsBurnable) {
		...
		CellRemover = std::make_shared<imstk::PbdObjectCellRemoval>(PbdObject);
	}
	
	if (SplineMeshMaterial && !SplineMeshMaterial->GetMaterial()->bUsedWithSplineMeshes)
		SubsystemInstance->LogToUnrealAndImstk("SplineMeshMaterial on connective tissue is not set to be used with spline meshes. Material will not be applied.", FColor::Yellow);
...
void UConnectiveTissue::UpdateModel()
{
	...
	if (bIsBurnable) {
		// This is normally handled in the imstk burning class but the cell removal doesnt occur for some reason
		auto burnStatePtr = std::dynamic_pointer_cast<imstk::DataArray<double>>(VisualGeom->getCellAttribute("BurnDamage"));
		imstk::DataArray<double>& burnState = *burnStatePtr;

		imstk::ParallelUtils::SpinLock lock;
		imstk::ParallelUtils::parallelFor(VisualGeom->getNumCells(),
			[&](const int cellId)
			{
				if (burnState[cellId] >= 1.0)
				{
					lock.lock();
					CellRemover->removeCellOnApply(cellId);
					//PbdObject->getPbdBody()->cellConstraintMap.erase(cellId);
					burnState[cellId] = 0.0;
					lock.unlock();
				}
			}, VisualGeom->getNumCells() > 50);
		CellRemover->apply();
	}
