...
void UPBDModel::Init()
{
	...
	if (GeomFilter.GeomType != EGeometryType::SurfaceMesh && !bRigidBody) {
		SubsystemInstance->LogToUnrealAndImstk("Non Rigid PBDModels can only be SurfaceMeshes", FColor::Red);
		return;
	}
	...
	if (!TetrahedralMesh) {
		...
		if (bCleanMesh && GeomFilter.GeomType == EGeometryType::SurfaceMesh) {
			...
			if (bCleanMesh && GeomFilter.GeomType != EGeometryType::SurfaceMesh)
				SubsystemInstance->LogToUnrealAndImstk("CleanMesh must be used with a surface mesh for " + Owner->GetName(), FColor::Yellow);
	...
	if (bScaleMassPerVertex && !bRigidBody)
