...
public:
	...
	*/
	...
	// Models must be shared for deformable models to interact
	UPROPERTY(EditAnywhere, Category = "iMSTK|Model")
		bool bSharedModel = true;
...
protected:
	...
	UPROPERTY(BlueprintReadWrite, Category = "iMSTK|DynamicalModel")
...
public:
	...
	*/
	...
	/** Gets the number of triangles if applicable, otherwise returns -1
	* @return int - The number of triangles present in the mesh. If the geometry does not have triangles then return -1
	*/
	UFUNCTION(BlueprintCallable, Category = "iMSTK|Mesh")
		virtual int GetNumTriangles();

	/** Gets the index array for the triangles in the visual geometry if applicable, otherwise returns and empty array.
	* @return TArray<int32> - Array containing the triangle indices of the geometry in one large array. If the geometry does not have triangles then return an empty array.
	*/
	UFUNCTION(BlueprintCallable, Category = "iMSTK|Mesh")
		virtual TArray<int32> GetTriangleIndices();

	/** Gets the number of vertices if applicable, otherwise returns -1
	* @return int - The number of vertices present in the geometry. If the geometry does not have vertices then return -1
	*/
	UFUNCTION(BlueprintCallable, Category = "iMSTK|PointSet")
		virtual int GetNumVertices();

	/** Gets an array containing the vertex positions of the visual geometry if applicable scaled to Unreal's size
	* @return TArray<FVector> - Array of FVectors containing the vertex locations. If the geometry does not have vertices then return an empty array.
	*/
	UFUNCTION(BlueprintCallable, Category = "iMSTK|PointSet")
		virtual TArray<FVector> GetVertexPositions();
};
